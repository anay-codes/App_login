...
export default function Reports() {
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/reports/${reportType}?format=json`);
  ...
  const loadDeptStats = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/reports/department-stats");
  ...
  const handleDownloadCSV = () => {
    window.open(`http://localhost:5000/api/reports/${reportType}?format=csv`, '_blank');
  };
